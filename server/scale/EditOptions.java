...
public class EditOptions extends Thread{
	...
	/* TO BE IMPLEMENTED in case 4 (?)
	public synchronized void editModelOptions()
	{
		int opSetIndex = auto.findOptionSet(args[0]);
		if(opSetIndex >= 0 && opSetIndex <= auto.getOptionSet().size())
		{
			System.out.print("Old model Choices: ");
			for(int i = 0; i < auto.getOptions(opSetIndex).size(); i++)
				System.out.print(auto.getOptions(opSetIndex));
